...
os.system("title WebSite Cloner - Clonador de Site")

os.system("color E")

print(""" L O A D I N G . . . | Code by Pixelzin """)
...
try:
    ...
    print("Use :\npython app.py www.example.com nome_da_pasta")
    sys.exit(1)
...
def save(bs, element, check):
    ...
    for l in links:
        ...
        if href is not None and href not in visited_links:
            if check in href:
                href = l.get("href")
                print("Transando com : {}".format(href))
                if "//" in href:
                    path_s = href.split("/")
                    file_name = ""
...
def crawl(link):
    ...
    if site_name in link and link not in visited_links:
        print("Transando com : {}".format(link))
        ...
        try:
            ...
            print("Error de conexão meu rapaz.")
            sys.exit(1)
